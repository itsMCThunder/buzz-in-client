...
function App() {
  ...
  if (!room) {
    return (<div className="bg-animated min-h-screen flex flex-col items-center justify-center text-white">
      ...
      <div className="mb-2">
        <label className="mr-4"><input type="radio" checked={mode==="freeplay"} onChange={()=>setMode("freeplay")}/> Free Play</label>
        <label><input type="radio" checked={mode==="teams"} onChange={()=>setMode("teams")}/> Teams</label>
      </div>
      <button onClick={createRoom} className="px-4 py-2 rounded-lg bg-blue-400 hover:bg-blue-500 font-bold shadow mb-2">Host</button>
      <input value={roomCode} onChange={(e)=>setRoomCode(e.target.value.toUpperCase())} placeholder="Room Code"
        className="p-2 rounded text-black mb-2"/>
      <button onClick={joinRoom} className="px-4 py-2 rounded-lg bg-green-400 hover:bg-green-500 font-bold shadow">Join</button>
    </div>);
  }

  return (<div className="bg-animated min-h-screen p-6 text-white">
    ...
    {isHost && room.mode==="teams" && !room.teams.A.name && (<div className="mb-4">
      <input value={teamA} onChange={(e)=>setTeamA(e.target.value)} placeholder="Team A" className="p-2 rounded text-black mr-2"/>
      <input value={teamB} onChange={(e)=>setTeamB(e.target.value)} placeholder="Team B" className="p-2 rounded text-black mr-2"/>
      <button onClick={setTeams} className="px-4 py-2 rounded-lg bg-blue-400 hover:bg-blue-500 font-bold shadow">Set Teams</button></div>)}
    <ul className="mb-4">{room.players.map(p=>(<li key={p.id} className="mb-1">
      {p.name} ({p.score}) {p.team && `[${room.teams[p.team].name}]`}
      {isHost && room.mode==="teams" && !p.team && (<><button onClick={()=>assignTeam(p.id,"A")} className="ml-2 px-2 py-1 bg-blue-400 rounded">Team A</button>
      <button onClick={()=>assignTeam(p.id,"B")} className="ml-2 px-2 py-1 bg-blue-600 rounded">Team B</button></>)}
      {isHost && room.mode==="freeplay" && (<><button onClick={()=>awardPoints(p.id,1)} className="ml-2 px-2 py-1 bg-green-400 rounded">+1</button>
      <button onClick={()=>awardPoints(p.id,-1)} className="ml-2 px-2 py-1 bg-red-400 rounded">-1</button></>)}</li>))}</ul>
    {isHost && (<div className="space-x-2 mb-4">
    ...
    {!isHost && (<button onClick={buzz} disabled={buzzersLocked} className="px-6 py-3 bg-yellow-400 text-black rounded-full text-lg font-bold shadow">Buzz!</button>)}
    {queue.length>0 && (<div><h3 className="font-bold">Queue:</h3><ol>{queue.map(id=>(<li key={id}>{room.players.find(p=>p.id===id)?.name}</li>))}</ol></div>)}
    {popup && (<div className="fixed top-0 left-0 right-0 bottom-0 bg-black bg-opacity-70 flex items-center justify-center">
      <div className="bg-blue-700 p-6 rounded-lg shadow-lg text-center">
        ...
        {isHost && <button onClick={nextRound} className="mt-4 px-4 py-2 bg-green-500 rounded-lg shadow">Next Round</button>}
      </div></div>)}
  </div>);
}
